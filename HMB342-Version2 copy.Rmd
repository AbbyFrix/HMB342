...
pc_attachment %>%
  ...
  mutate(attached_rate_80_plus = (attached_80_plus/total) * 1000) -> pc_attachment_75
...
edv_attachment_75_comb %>%
  ggplot(aes(x = attached_rate_80_plus)) +
  ...
  theme_minimal() +
  labs(x = "Rate of Primary Care Attachment (per 1000 population)", y = "Average Annual Rate of ED Visits (per 1000 population)")
```
...
pc_attachment %>%
  ...
  mutate(attached_rate_65_79 = (attached_65_79/total) * 1000) -> pc_attachment_65_79
...
edv_attachment_65_79_comb %>%
  ggplot(aes(x = attached_rate_65_79)) +
  geom_point(aes(y = high_urgency_65_79, color = "high urgency")) +
    geom_point(aes(y = low_urgency_65_79, color = "low urgency"))+
    scale_color_manual(name = "Emergency Department Visit Urgency", values = c("high urgency" = "red3", "low urgency" = "orange3")) +
  theme_minimal() +
  labs(x = "Rate of Primary Care Attachment (per 1000 population)", y = "Average Annual Rate of ED Visits (per 1000 population)")
```

```{r}
lmmodel65_79 <- glm(high_urgency_65_79 ~ attached_rate_65_79, data = edv_attachment_65_79_comb, family = "gaussian")
summary(lmmodel65_79)
AIC(lmmodel65_79)
r.squaredGLMM(lmmodel65_79)


min_value <- min(edv_attachment_65_79_comb$attached_rate_65_79, na.rm = TRUE)
max_value <- max(edv_attachment_65_79_comb$attached_rate_65_79, na.rm = TRUE)


new_data <- expand.grid(
  attached_rate_65_79 = seq(
    min(min_value),
    max(max_value),
    length.out = 100
  )
...
```{r}
ggplot(edv_attachment_65_79_comb, aes(x = attached_rate_65_79, y = high_urgency_65_79)) +
  geom_point(color = "red3", alpha = 0.6) +  
  geom_line(data = new_data, aes(x = attached_rate_65_79, y = predict), color = "black", size = 1.2) +  
  labs(
    title = "Relationship between Primary Care Attachment and High Urgency ED Visits (65-79)",
    x = "Rate of Primary Care Attachment (per 1000 population)",
...
pc_attachment %>%
  ...
  mutate(attached_rate_45_64 = (attached_50_64/total) * 1000) -> pc_attachment_45_64
...
edv_attachment_45_64_comb %>%
  ggplot(aes(x = attached_rate_45_64)) +
  geom_point(aes(y = high_urgency_45_64, color = "high urgency")) +
    geom_point(aes(y = low_urgency_45_64, color = "low urgency"))+
    scale_color_manual(name = "Emergency Department Visit Urgency", values = c("high urgency" = "red3", "low urgency" = "lightblue")) +
  theme_minimal() +
  labs(x = "Rate of Primary Care Attachment (per 1000 population)", y = "AAverage Annual Rate of ED Visits (per 1000 population)")
```
...
pc_attachment %>%
  ...
  mutate(attached_rate_35_49 = (attached_35_49/total) * 1000) -> pc_attachment_35_49
...
edv_attachment_35_49_comb %>%
  ggplot(aes(x = attached_rate_35_49)) +
...
pc_attachment %>%
  ...
  mutate(attached_rate_20_34 = (attached_19_34/total) * 1000) -> pc_attachment_20_34
...
edv_attachment_20_34_comb %>%
  ggplot(aes(x = attached_rate_20_34)) +
...
pc_attachment %>%
  ...
  mutate(attached_rate_under_19 = (attached_under_19/total) * 1000) -> pc_attachment_under_19
  


edv_attachment_5_19_comb <- full_join(edv_low_high_5_19, pc_attachment_under_19,
            by = c("neighborhood_name"))
...
edv_attachment_5_19_comb %>%
  ggplot(aes(x = attached_rate_under_19)) +
...
edv_attachment_0_4_comb <- full_join(edv_low_high_0_4, edv_attachment_5_19_comb,
            by = c("neighborhood_name"))
...
```{r}
edv_attachment_under_19 <- full_join(edv_attachment_0_4_comb, edv_attachment_5_19_comb, by = "neighborhood_name")
```
...
```{r}
edv_attachment_all_age_comb <- edv_attachment_under_19 %>%
  full_join(edv_attachment_20_34_comb, by = "neighborhood_name") %>%
  full_join(edv_attachment_35_49_comb, by = "neighborhood_name") %>%
  full_join(edv_attachment_45_64_comb, by = "neighborhood_name") %>%
  full_join(edv_attachment_65_79_comb, by = "neighborhood_name") %>%
  full_join(edv_attachment_75_comb, by = "neighborhood_name")

edv_attachment_all_age_comb <- edv_attachment_all_age_comb %>%
  rename(
    attached_rate_75_plus = attached_rate_80_plus
  )
edv_attachment_all_age_comb
```
...
edv_long <- edv_attachment_all_age_comb %>%
  pivot_longer(
    cols = c(
      ...
      "low_urgency_5_19.x", "high_urgency_5_19.x", 
...
edv_long %>%
  ...
  mutate(age_group = factor(age_group, 
                            levels = c("0_4","5_19", "20_34", "35_49", "45_64", "65_79", "75_plus"))) %>%
  ...
  labs(
    ...
    fill = "Visit Type") + 
    scale_fill_manual(values = c("red3", "orange3")) + 
  theme_minimal()
...
pc_by_age<- pc_attachment_under_19 %>%
  full_join(pc_attachment_20_34, by = "neighborhood_name") %>%
  full_join(pc_attachment_45_64, by = "neighborhood_name") %>%
  full_join(pc_attachment_65_79, by = "neighborhood_name") %>%
  full_join(pc_attachment_75, by = "neighborhood_name")

pc_by_age <- pc_by_age %>%
  pivot_longer(
    cols = c(
      "attached_rate_under_19",
      "attached_rate_20_34",
      "attached_rate_45_64",
      "attached_rate_65_79",
      "attached_rate_80_plus"
    ),
    names_to = "age_group",
    values_to = "attachment_rate"
  ) %>%
  mutate(age_group = case_when(
    age_group == "attached_rate_under_19" ~ "under_19",
    age_group == "attached_rate_20_34" ~ "20_34",
    age_group == "attached_rate_45_64" ~ "45_64",
    age_group == "attached_rate_65_79" ~ "65_79",
    age_group == "attached_rate_80_plus" ~ "75_plus"
  ))

pc_by_age %>%
   mutate(age_group = factor(age_group, 
                            levels = c("under_19", "20_34", "45_64", "65_79", "75_plus"))) %>%
  ggplot(aes(x = age_group, y = attachment_rate)) + 
  geom_boxplot(position = position_dodge(0.8),fill = "gold") + 
  labs(
    title = "Rate of Primary Care Attachment Stratified by Age", 
    x = "Age Group", 
    y = "Rate of Primary Care Attachment (per 1000 population)") + 
  theme_minimal()
```
